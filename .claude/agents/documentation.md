...
skills:
  - python-docs
  - doc-audit
tools: Read, Write, Edit, Grep, Glob, mcp__serena, mcp__context7, WebSearch
---

# Documentation Agent

Responsible for maintaining accurate, objective, and complete documentation for the Drift project.
...
- Current CLI flags (e.g., `--no-llm` not `--skip-llm`)

## Drift-Specific Validation

### CLI Examples
Verify all examples use valid flags:
- `drift` - Basic usage
- `drift --no-llm` - Programmatic validation only
- `drift --days 7` - Time-based filtering
- `drift --format json` - Output format
- `drift --cwd PATH` - Working directory

### Configuration Examples
Validate configuration file structure:
- Provider types: `anthropic`, `bedrock`
- Rule names match actual validators
...
- Invalid CLI flags in examples
- Incorrect configuration file structure
...
- CLI examples use only valid flags
